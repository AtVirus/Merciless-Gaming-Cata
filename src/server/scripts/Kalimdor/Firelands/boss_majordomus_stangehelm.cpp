...
- Check Display IDs of the Cat and Scorpion Form
 ...
 Updated to TrinityCore 4.3.4 by
        __      ___                
    ___\ \    / (_)               
   / __ \ \  / / _ _ __ _   _ ___ 
  / / _` \ \/ / | | '__| | | / __|
...
public:
	...
	{
		boss_majordomusAI(Creature* c) : BossAI(c, DATA_MAJORDOMUS_STANGEHELM)
		{
		...
		void Reset()
		{
			...
			instance->SetBossState(DATA_MAJORDOMUS_STANGEHELM, NOT_STARTED);
			events.Reset();
		...
		{
			instance->SetBossState(DATA_MAJORDOMUS_STANGEHELM, DONE);
		...
		void UpdateAI(uint32 diff) //const 
		{
			...
			if(phase == PHASE_CAT && me->GetPower(POWER_ENERGY) == 100)
			{
				...
				me->SummonCreature(52593 ,me->GetPositionX(),me->GetPositionY()
