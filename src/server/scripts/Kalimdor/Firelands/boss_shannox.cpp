...
{
		PHASE_NON						= 1,
		PHASE_SHANNOX_HAS_SPEER			= 2,
		PHASE_SPEAR_ON_THE_GROUND		= 3,
		PHASE_RIPLIMB_GOS_TO_SPEER		= 4,
		PHASE_RIPLIMB_BRINGS_SPEER		= 5,
};
	...
	struct boss_shannoxAI : public BossAI
	{
		...
		void DoAction(int32 action)
        {
			...
			case ACTION_SAY_RIPLIMB:
				DoPlaySoundToSet(me, SAY_RIPLIMB);
                break;
            }
        }
		...
		void UpdateAI(uint32 diff)
		{
			...
			if( (!introSpeechDone) && (!me->isInCombat()) )
			{
				...
				DoPlaySoundToSet(me, SAY_INTRO_SPECH_PART_ONE);
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_SHANNOX_SEC_INTRO_YELL:
					DoPlaySoundToSet(me, SAY_INTRO_SPECH_PART_TWO);
					break;
				...
				case EVENT_HURL_SPEAR_OR_MAGMA_RUPTUTRE:
					...
					}else
					{
						...
						if (instance->GetData(DATA_CURRENT_ENCOUNTER_PHASE) == PHASE_SHANNOX_HAS_SPEER)
						{
							...
							DoPlaySoundToSet(me, RAND(SAY_BURN_ONE,SAY_BURN_TWO));
			...
			if ((pRiplimb->isDead() || pRageface -> isDead()) && !softEnrage)
			{
				...
				DoPlaySoundToSet(me, SAY_ON_DOGS_FALL); //TODO Test
